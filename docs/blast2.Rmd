...
Get data for mitochondria from gibbons and chimps. Build a blast database from one and query the other. Then build a map of the blast hits.
...
curl https://raw.githubusercontent.com/rsh249/bioinformatics/master/data/hylobates.fasta > data/vulpes.fa
```
...
ncbi-blast-2.7.1+/bin/blastn.exe -db data/pan.fa -query data/vulpes.fa -outfmt 6 -max_target_seqs 1000 -evalue 10 -word_size 8  -out comp.out    
...
ge <- read_dna_seg_from_fasta('data/vulpes.fa')
trvsge <- read_comparison_from_blast('comp.out')


plot_gene_map(dna_segs=list(tr, ge),
  comparisons=list(trvsge),
  main="Chimp and Wolf Mitochondria",
  gene_type="side_blocks",
  dna_seg_scale=TRUE, 
  scale=TRUE)
```
